                    ...
                    let direction = object
                        ...
                        .or_else(|| {
                            props.get("port.direction")
                                .and_then(|value| PortDirection::parse(value))
                        });
        ...
        self.nodes
            ...
            .or_else(|| {
                self.nodes.values().find(|node| {
                    matches_prop(node, "node.description", sink_description)
                        || matches_prop(node, "device.description", sink_description)
                })
            })
    }
}
...
impl PatchbayState {
    ...
    fn ensure_virtual_sink(&mut self) -> Result<VirtualSinkInfo> {
        ...
        logger::info(&format!(
            "[patchbay] creating virtual sink {}",
            self.sink_name
        ));
        ...
        let module_id = module_id_text
            .trim()
            .parse::<u32>()
            .map_err(|err| BackendError::InvalidOutput {
                program: "pactl",
        ...
        loop {
            if let Some(info) = self.virtual_sink_info()? {
                logger::info(&format!(
                    "[patchbay] virtual sink ready: {} ({})",
                    info.sink_name, info.node_id
                ));
                return Ok(info);
            }
    ...
    fn route_nodes(&mut self, node_ids: Vec<u32>) -> Result<VirtualSinkInfo> {
        ...
        if node_ids.is_empty() {
            return Err(BackendError::Message(
                "at least one node id is required".to_string(),
            ));
        }

        let snapshot = PipeWireSnapshot::collect()?;
        let Some(sink_node) = snapshot.find_virtual_sink(&self.sink_name, &self.sink_description)
        else {
            return Err(BackendError::Message(
        ...
        if sink_inputs.is_empty() {
            return Err(BackendError::Message(
                "virtual sink has no usable input ports".to_string(),
            ));
        }
        ...
        if created == 0 {
            return Err(BackendError::Message(
                "none of the selected nodes could be linked".to_string(),
            ));
        }
        ...
        let snapshot = PipeWireSnapshot {
            nodes: HashMap::from([
                (
                    ...
                    node_with_props(
                        1,
                        &[("node.description", "Vencord Screen Share"), ("node.name", "other")],
                    ),
                ),
                (
                    2,
                    node_with_props(
                        2,
                        &[
                            ("node.description", "Something Else"),
                            ("node.name", "patchcord-screen-share-123"),
                        ],
                    ),
                ),
            ]),
